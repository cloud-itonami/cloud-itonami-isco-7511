(ns butcher.actor
  "ButcherActor — the ISCO-08 7511 butcher-shop/fishmonger shop
  scheduling/logistics coordination actor as a `langgraph.graph/state-graph`
  (ADR-2607121000 / CLAUDE.md Actors section). One graph run = one shop-
  ...
  cloud-itonami-isco-7213's sheetmetal.actor for the cutting-tool
  ...
  The unconditional invariant: the Butcher/Fishmonger Advisor can never
  directly commit an operating record the ButcherGovernor refuses, and
  can never make this actor finalize a preparation-execution decision
  (e.g. a specific cutting or butchering operation) or a food-safety-
  clearance decision (e.g. declaring a batch fit for sale), and can never
  override a shop safety officer's judgment — every commit-record! call
  is gated behind `:decide`, and both scope-excluded decision classes are
  ...
  butchering/preparation work itself and never makes a food-safety-
...
(defn build-graph
  "Build a compiled ButcherActor graph. `store` implements
  `butcher.store/Store`. `advisor` implements `butcher.advisor/Advisor`
