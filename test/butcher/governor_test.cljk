(ns butcher.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [butcher.store :as store]
            [butcher.advisor :as advisor]
            [butcher.governor :as governor]))

(defn- fresh-store []
  (let [st (store/mem-store)]
    (store/register-worker! st {:worker-id "worker-1" :name "Kenji Sato"})
    (store/register-shop! st {:shop-id "SHOP-1" :name "Sato Butcher Shop" :max-supply-cost 2000})
    st))

(defn- op [op-kw & {:as extra}]
  (merge {:op op-kw :effect :propose :shop-id "SHOP-1"
...
(deftest hard-on-unregistered-shop
  (let [st (fresh-store)
        v (governor/check req {} (op :log-work-record :shop-id "SHOP-ghost") st)]
    (is (:hard? v))
...
(deftest hard-on-scope-excluded-op-finalize-cutting-operation
  (testing "finalizing a preparation-execution decision (cutting operation) is a permanent block, never a routine op"
    (let [st (fresh-store)
...
(deftest hard-on-scope-excluded-op-proceed-with-butchering-operation
  (testing "proceeding directly with a butchering operation is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :proceed-with-butchering-operation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-declare-batch-fit-for-sale
  (testing "declaring a batch fit for sale (a food-safety-clearance decision) is a permanent block, never a routine op — CLAUDE.md's food-safety-clearance dimension"
    (let [st (fresh-store)
...
(deftest hard-on-scope-excluded-rationale-finalize-cutting-operation
...
(deftest hard-on-scope-excluded-rationale-declare-batch-fit-for-sale
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a cut-hazard/hygiene-compliance/contamination-risk concern always requires human sign-off"
    (let [st (fresh-store)
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'cutting'/'batch'/'shop safety officer', but never the full finalization-action phrases)"
    (let [st (fresh-store)
          adv (advisor/mock-advisor)
          ops [:log-work-record :schedule-crew-operation
               :flag-safety-concern :coordinate-supply-order]]
      (doseq [o ops]
        (let [request {:worker-id "worker-1" :op o :shop-id "SHOP-1"
